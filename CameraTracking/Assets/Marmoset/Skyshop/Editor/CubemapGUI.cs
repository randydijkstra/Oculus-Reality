...
namespace mset {
	public class CubemapGUI : ScriptableObject {
		...
		public void newCube() {
			...
			input = cube;
			mset.Util.setLinear(new SerializedObject(input), false);
		...
		public void setLinear( bool yes ) {
			if(srInput != null) mset.Util.setLinear(srInput, yes);
		...
		private void fixInput() {
			...
			mset.Util.setReadable(srInput, true);
			...
			if( is2D ) {
				//limits on Texture2D
				TextureImporter ti = mset.Util.getTextureImporter(ipath, "CubemapGUI.fixInput");
				...
				npotScale = false;
				tooSmall = false;
				linear = mset.Util.isLinear(srInput);
				compressed = !mset.Util.isUncompressed(srInput);
				mipped = mset.Util.isMipmapped(srInput);
			}
			...
			if( linear || compressed || (mipped != this.mipmapped) || tooSmall || npotScale ) {
				...
				if( EditorUtility.DisplayDialog("Fix Texture Format?", "The chosen texture has a few formatting problems:" + problems, "Fix Now", "Ignore") ) {
					if( is2D ) {
						...
						mset.Util.setUncompressed(srInput);
						mset.Util.setMipmapped(srInput, this.mipmapped);
					}
				...
				if( bAsInput ) {
					...
					if( GUILayout.Button(new GUIContent("Use as Input",useInputTip), buttonStyle, GUILayout.Width(85), GUILayout.Height(buttonHeight)) ) {
						if( this.inputGUI != null ) {
							...
							this.inputGUI.setReference(AssetDatabase.GetAssetPath(this.cube), false, true);
						}
					}
					EditorGUI.EndDisabledGroup();
				}
