...
namespace mset {
	...
	public class Util {
		...
		*/
		...
		/* TODO: this is the wrong property or something
		public static int getMaxSize(SerializedObject serialTex) {
			if(serialTex == null) return 0;
			SerializedProperty prop = serialTex.FindProperty("m_TextureDimension");
			if( prop != null ) {
				return (1 << (12-prop.intValue));
			}
			Debug.LogError("m_TextureDimension SerializedProperty not found!");
			return 0;
		}
		public static void setMaxSize(SerializedObject serialTex, int size) {
			SerializedProperty prop = serialTex.FindProperty("m_TextureDimension");
			if( prop != null ) {
				int logSize = Mathf.Clamp(mset.QPow.Log2i(size), 5, 12); // between 32 and 4096
				//int log4096 = 12;
				Debug.Log ("@@@ trying to set textureDimension to " + logSize);
				prop.intValue = 1;
				serialTex.ApplyModifiedProperties();
			}
			else {
