...
public class Skyshop : EditorWindow {
	...
	private void uploadReflection(int mip, int mipSize) {
		ps.IN.resampleToCube(ref outSIM.cube, mip, outSIM.colorMode, needsGamma(outSIM));
	}

	private void uploadMipLevel(int mip, int mipSize) {
		ps.CONVO.resampleToCube(ref outSIM.cube, mip, outSIM.colorMode, needsGamma(outSIM));
	}

	private void finishSKY() {
		if( outSKY.cube && inSKY.input != outSKY.cube ) {
			ps.IN.resampleToCube(ref outSKY.cube, 0, outSKY.colorMode, needsGamma(outSKY));
	...
	private void finishDIM() {
		...
		ps.CONVO.resampleToCube(ref outDIM.cube, 0, outDIM.colorMode, needsGamma(outDIM));
		outDIM.locked = false;
		outDIM.setReference(cubePath, false, true);
		outDIM.setLinear(needsLinearSampling(outDIM));
	...
	private void finishSIM() {
		...
		if( !ps.buildMipChain ){
			ps.CONVO.resampleToCube(ref outSIM.cube, 0, outSIM.colorMode, needsGamma(outSIM));
